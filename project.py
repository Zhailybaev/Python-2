...
def ref(window,a):
    ...
    for i in range(n):
        ...
        for j in range(i+1, n):
            if a[i][i]!=0:
                ...
                a[j]=np.around(a[j]- ratio * a[i],decimals=2)
                Label(window_solve, text=f'{np.around(ratio,decimals=2)}').place(x=10+x3 , y=50)
                print(a)
            else:
              w=tuple(a[i])
              swap(a,i,w)
    ...
    lu=np.matmul(L,a)
    lu1=np.around(lu,decimals=1)
    x3=30
    y3=240
    d.view(window_solve, lu1,x3,y3)
